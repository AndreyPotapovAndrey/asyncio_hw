...
async def main():
    ...
    for people_id_chunk in chunked(range(1, 10), CHUNK_SIZE):  # Разбиваем на последовательности по 10
        ...
        asyncio.create_task(insert_to_db(result))  # Объект задач (create_task) формируется на базе карутин и начинает
    ...
    set_of_tasks = asyncio.all_tasks() - {asyncio.current_task()}  # Исключаем из сета задач main
